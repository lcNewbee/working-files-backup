...
class AccountList_Model extends CI_Model {
    ...
    function Recharge($data){
        ...
        if($result){
            $this->addUserRecharge($data['id'],$payType,$payTime,$row->state,$row->time);
        }
        $result ? $result = json_ok() : $result = json_no('insert error');
        return json_encode($result);
    }
    private function addUserRecharge($id,$payType,$payTime,$state,$sum){
        /*
    ...
    //socket portal
    private function notice_socket($data){
        $result = null;
        $portal_socket = new PortalSocket();
