...
export default class SystemStatus extends React.Component {
  ...
  render() {
    ...
    console.log('this.props.selfState.currRadioConfig', this.props.selfState);
    ...
    const { status, wirelessMode } = this.props.store.getIn(['curData', 'radioList', radioId]).toJS();
    const {
      deviceModel, deviceName, networkMode, security, version, systemTime, frequency, channelWidth,
      uptime, ap, channel, interfaces, vap_interfaces, station, wlan0Mac, protocol, lan0Mac, lan1Mac,
                ...
                />
                {
                  ap === undefined ? null : (
                    <FormGroup
                      ...
                      id="userNumber"
                      type="plain-text"
                      value={this.props.store.getIn(['curData', 'radioList', radioId, 'status', 'ap', 'clientNum'])}
                    />
                  )
                }
              </div>
